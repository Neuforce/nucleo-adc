...
| **Chip del tipo — dónde está el indicador en la cadena** | `2 RUMBO` | `TipoIndicadorClave` / `TipoIndicadorNombre` | MDM → `dim.Indicador`. **Es una secuencia del 1 al 5** (ver §2.2) |
...
| `TipoIndicador` — la cadena | Qué significa | Activos |
|---|---|---|
| **1 META** | El resultado que se persigue. Final de la cadena | 34 |
| **2 RUMBO** | Dice si vamos hacia la meta o nos desviamos | 29 |
| **3 EMPUJE** | Lo que se hace para mover el rumbo | 161 |
| **4 ARRANQUE** | Las condiciones previas para poder empujar | 93 |
| **5 REFERENCIA** | Contexto: no se persigue, se consulta | 103 |
...
- **El tipo es ordinal y su número significa algo**: sirve para ordenar el tablero y leerlo
  como cadena. La clase **no** tiene orden; encodearla como escala inventa una jerarquía.
- **Todo `5 REFERENCIA` es clase `Referencia`**: ahí el segundo chip es redundante y
  conviene colapsarlo.
